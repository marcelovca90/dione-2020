...
while percent < 1.0:
    ...
    while p_nu > p_nu_limit:
        ...
        while p_words < p_words_limit:
            ...
            print("{}\tpercent: {:.2f}\tp_words: {}\tnu: {}\tgamma: {}\taccuracy: {:.4f}\tprecision: {:.4f}\trecall: {:.4f}\tf1: {:.4f}".format(dt_str, percent, p_words, clf.nu, clf.gamma, np.mean(accuracy_score), np.mean(precision_score), np.mean(recall_score), np.mean(f1_score)))
